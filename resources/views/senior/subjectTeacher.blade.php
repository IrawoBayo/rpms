...
<div class="row">
	<div class="col-lg-12">
		<section class="panel panel-default">
			...
			<form action="{{ route('ss-subjectTeacher') }}" class="form-horizontal" id="frm-create-subject" method="POST">
				{{ csrf_field() }}
								...
								@foreach($subjectteachers as $st)
									@if($st->user->active==1)
										<tr>
											...
											<td>{{ $st->user->name }}</td>
											<td>
												<a href="{{ url('/ss-editST/'.$st->id) }}"><button class="btn btn-info">Edit</button></a>
												<a href="{{ url('/ss-deleteST/'.$st->id) }}"><button onclick="return confirm('Are you sure you want to delete this Subject Teacher?')" class="btn btn-danger">Delete</button></a>
											</td>
										</tr>
									@endif
								@endforeach

...
<div class="row">
	<div class="col-lg-12">
		<section class="panel panel-default">
			...
			<form action="{{ route('ss-classTeacher') }}" class="form-horizontal" id="frm-create-subject" method="POST">
				{{ csrf_field() }}
								...
								@foreach($classteachers as $ct)
									@if($ct->user->active==1)
										<tr>
											...
											<td>{{ $ct->user->name }}</td>
											<td>
												<a href="{{ url('/ss-editCT/'.$ct->id) }}"><button class="btn btn-info">Edit</button></a>
												<a href="{{ url('/ss-deleteCT/'.$ct->id) }}"><button onclick="return confirm('Are you sure you want to delete this Class Teacher?')" class="btn btn-danger">Delete</button></a>
											</td>
										</tr>
									@endif
								@endforeach

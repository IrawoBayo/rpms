...
<div class="row">
	<div class="col-lg-12">
		<section class="panel panel-default">
			...
			<form action="{{ route('pri-classTeacher') }}" class="form-horizontal" id="frm-create-subject" method="POST">
				{{ csrf_field() }}
						...
						<table class="table table-stripped" id="myTable">
							...
							<tbody>
								@foreach($classteachers as $ct)
									@if($ct->user->active==1)
										<tr>
											<td>{{ $ct->classes->class_name }}{{ $ct->classes->section }}</td>
											<td>{{ $ct->user->name }}</td>
											<td>
												<a href="{{ url('/pri-editCT/'.$ct->id) }}"><button class="btn btn-info">Edit</button></a>
												<a href="{{ url('/pri-deleteCT/'.$ct->id) }}"><button onclick="return confirm('Are you sure you want to delete this Class Teacher?')" class="btn btn-danger">Delete</button></a>
											</td>
										</tr>
									@endif
								@endforeach

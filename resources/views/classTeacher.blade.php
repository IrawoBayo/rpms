...
<div class="row">
	<div class="col-lg-12">
		<section class="panel panel-default">
			...
			<form action="{{ route('classTeacher') }}" class="form-horizontal" id="frm-create-subject" method="POST">
				{{ csrf_field() }}
								...
								@foreach($classteachers as $ct)
									@if($ct->user->active==1)
										<tr>
											...
											<td>{{ $ct->user->name }}</td>
											<td>
												<a href="{{ url('/editCT/'.$ct->id) }}"><button class="btn btn-info">Edit</button></a>
												<a href="{{ url('/deleteCT/'.$ct->id) }}"><button class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this?')">Delete</button></a>
											</td>
										</tr>
									@endif
								@endforeach
